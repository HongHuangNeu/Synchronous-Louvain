...
//manipulate community.dat
    ...
    //val textFile1 = sc.textFile(Setting.graphHome+"/con.dat")
...
//manipulate the community I produce    
    val textFile2 = sc.textFile(Setting.graphHome+"/level1")
    //val textFile2 = sc.textFile(Setting.graphHome+"/le1")
    ...
    ijMap.foreach(f=>{
       sum=sum+f._2.toDouble*log2(f._2.toDouble*N.toDouble/(cluster1Map(f._1._1).toDouble*cluster2Map(f._1._2).toDouble))
    })
    
    var sum1=0.0
    cluster1Map.foreach(f=>{
      sum1=sum1+f._2.toDouble*log2(f._2.toDouble/N.toDouble)
    })
    cluster1Map.foreach(f=>println(f))
    var sum2=0.0
    cluster2Map.foreach(f=>{
      sum2=sum2+f._2.toDouble*log2(f._2.toDouble/N.toDouble)
    })
